...
export const BACKUP_APP_ID = 'the-missing-parts'
...
export function backupFilename(now = new Date()): string {
  ...
  return `the-missing-parts-backup-${stamp}.json`
}

/** The text of one section, used for comparing two records. */
...
export function analyseImport(text: string, existing: Entry[]): ImportPreview {
  ...
  if (Array.isArray(parsed)) {
    ...
    if (typeof record.app === 'string' && record.app !== BACKUP_APP_ID) {
      warnings.push(`This backup says it came from “${record.app}”, not from The Missing Parts.`)
    }
  }

  if (!rawEntries) {
    return emptyPreview([
      'No list of entries was found in this file. A backup from The Missing Parts has an “entries” list.',
    ])
  }
  ...
  for (const incoming of entries) {
    ...
    for (const section of SECTIONS) {
      ...
      if (hasIncoming && !hasExisting) adds = true
      else if (hasIncoming && hasExisting && sectionText(incoming, section) !== sectionText(match, section)) {
