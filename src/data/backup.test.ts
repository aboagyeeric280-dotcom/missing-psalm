...
describe('self-check 8: merge preserves current entries and reports conflicts', () => {
  ...
  it('keeps existing wording by default and adds the new record', () => {
    ...
    expect(entries.find((item) => item.id === 'a')?.readingText).toBe(READING.readingText)
    expect(entries.find((item) => item.id === 'b')?.concludingPrayer).toBe(PRAYER.concludingPrayer)
  ...
  it('takes the imported wording when the user asks for it', () => {
    ...
    const { entries } = applyImport([READING, PRAYER], preview, {
      mode: 'merge',
      conflictWinner: 'imported',
    })
    expect(entries.find((item) => item.id === 'a')?.readingText).toBe('A different wording of the same reading.')
    expect(entries).toHaveLength(3)
  })
  ...
  it('replace mode swaps the whole list and says what was removed', () => {
    ...
    expect(entries.find((item) => item.id === 'b')).toBeUndefined()
  })
})
